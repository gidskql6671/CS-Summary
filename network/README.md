...
## OSI 7계층
네트워크 통신 프로토콜을 기능별로 7단계로 나눈 것이다. IOS에서 상호 이질적인 네트워크간의 연결에서 호환성의 결여를 막기 위해 OSI 레이어를 개발하였다. 네트워크 통신간의 데이터 표준 혹은 프로토콜을 정리한 것이라 생각하면 된다.
...
## TCP/IP 4 계층
위에서 설명한 OSI 7계층은 네트워크 통신간 지켜야한 표준/프로토콜이다. 이를 기반으로 만든 실제 사용하는 인터넷 표준이 TCP/IP 4계층이다. OSI 7계층을 기반으로 상업적이고 실무적으로 이용될 수 있도록 단순화하였다.  

![tcp/ip 4계층](./images/tcpip_4계층.png) 
[TCP/IP 4계층(TCP/IP 4 Layer)](https://hahahoho5915.tistory.com/15)

위 그림처럼 OSI 7 계층의 여러 계층을 하나로 묶은 모습을 볼 수 있다.

### 1계층: 네트워크 엑세스 계층(Network Access Layer)
OSI 7계층의 물리 계층과 데이터링크 계층에 해당한다. Nodo-To-Node 간의 신뢰성있는 데이터 전송을 담당한다.  
물리적인 주소로 **MAC Address**를 가지며, 이 주소를 사용해 통신을 한다.  
사용되는 데이터 단위는 프레임이며, 헤더로 Mac Address를 넣는다. LAN, 패킷망 등에 사용되며, 주요 프로토콜로는 이더넷, HDLC 등이 있다.

### 2계층: 인터넷 계층(Internet Layer)
OSI 7계층의 네트워크 계층에 해당한다.  
호스트 간의 라우팅을 담당하며, 통신 노드간의 IP 패킷을 전송한다.  
사용되는 데이터 단위는 패킷이며, 헤더로 IP Address를 넣는다.  
주요 프로토콜로는 IP, ARP, RARP 등이 있다.

### 3계층: 전송 계층(Transport Layer)
OSI 7계층의 전송 계층에 해당한다.  
프로세스 간의 신뢰성있는 데이터 전송을 담당한다. 각 통신 노드 간의 연결을 제어한다.  
데이터의 단위는 TCP일 시 Segment, UDP일 시 Datagram이다. 또한 헤더로는 Port Number를 넣는다.  
주요 프로토콜로는 TCP, UDP가 있다.

### 4계층: 응용 계층(Application Layer)
OSI 7계층의 세션 계층, 표현 계층, 응용 계층에 해당한다.  
사용자와 가장 가짜운 계층이며, 서버나 클라이언트의 응용 프로그램이 이 계층에서 동작한다.  
각 프로세스들은 포트 번호를 통해 구별된다.  
주요 프로토콜로는 HTTP, SSH, FTP 등이 있다.